...
(function () {
   ...
   * @param {HTMLVideoElement} originalVideo - The original video element
   * @param {string} entryId - The video entry ID
   * @param {string} [sourceUrl] - Optional override source URL (for HLS manifests)
   */
  function createSinglePreview(originalVideo, entryId, sourceUrl) {
    console.log(`[Preview] Creating preview video element for ${entryId}`);

    // Use provided source URL or fall back to original video source
    const rawSrc =
      sourceUrl || originalVideo.currentSrc || originalVideo.src || "";
    ...
    let videoUrl = rawSrc;
    // Add preview parameter for non-HLS/DASH sources only
    if (
      videoUrl &&
    ...
    console.log(
      `[Preview] 📺 Preview source for ${entryId}: ` +
        `${isHLS ? "HLS" : isDASH ? "DASH" : isBlob ? "BLOB (pending resolution)" : "Standard"} | ` +
    ...
    preview.dataset.previewSrc = isHLS
      ? "hls-manifest"
      : isBlob
        ? "blob-pending"
        : "direct";
    ...
    // For HLS sources, set the type attribute to help the browser
    if (isHLS) {
      preview.dataset.hlsSource = "true";
      console.log(`[Preview] 📡 HLS preview source detected for ${entryId}`);
    }

    // For blob sources, mark as pending resolution
    if (isBlob) {
      preview.dataset.sourcePending = "true";
      console.log(
        `[Preview] ⏳ Blob source pending HLS manifest resolution for ${entryId}`,
      );
    }
    ...
    function initializePreview() {
      ...
      console.log(
        `[Preview] Initializing preview for ${entryId} | readyState: ${preview.readyState} | duration: ${preview.duration || "N/A"}`,
      );
    ...
    // For blob sources that are pending resolution, listen for src changes
    if (isBlob) {
      console.log(
        `[Preview] 🔄 Setting up source change listener for ${entryId}`,
      );
      const srcObserver = new MutationObserver(() => {
        const newSrc = preview.currentSrc || preview.src || "";
        if (
          newSrc &&
          ...
          console.log(
            `[Preview] 🎯 Source resolved for ${entryId}: ${newSrc.substring(0, 80)}...`,
          );
          preview.dataset.sourcePending = "false";
          srcObserver.disconnect();
          // Update cache key
          const newCleanSrc = newSrc.split("?")[0];
          preview.dataset.cacheKeySrc = newCleanSrc;
          if (entry) entry.cacheKeySrc = newCleanSrc;
    ...
    if (preview.readyState >= 1 && preview.duration > 0 && !isBlob) {
      initializePreview();
    } else {
      ...
      metadataTimeout = setTimeout(() => {
        if (!isInitialized && document.body.contains(preview)) {
          const currentSrc = preview.currentSrc || preview.src || "";
          if (currentSrc.startsWith("blob:")) {
            console.warn(
              `[Preview] ⚠️ Metadata timeout for ${entryId} (blob source - waiting for HLS resolution)`,
            );
            // Don't initialize yet - wait for source resolution
          } else {
            console.warn(
